...
🇬🇧 [English version](../en/training.md)
...
На практике: **mAP50** удобно использовать как быстрый основной индикатор качества, а **mAP50-95** — как более строгую метрику, которая лучше отражает качество боксов на разных порогах IoU.
...
Быстрое правило:
- **NCNN** — для деплоя на Raspberry Pi;
- **`.pt`** — для обучения, отладки и GPU-хостов;
- **ONNX** — только если нужна совместимость с другими runtime/framework.
...
|---|---|---|
| [Подготовка датасета](dataset.md) | [Вернуться к README](README.md) | [Запуск проекта](launch.md) |
