...
INT8 быстрее только на 1 потоке (+5%). На 2-4 потоках FP32 быстрее, потому что overhead dequantize съедает выигрыш от параллелизации. Главное преимущество INT8: размер модели 2.6 MB (на 75% меньше).
