...
Если mDNS или локальный DNS не всегда стабильно резолвят `rpi5` / `blackops`, добавь статические алиасы в `/etc/hosts` на своей dev-машине:
...
Тогда hostname'ы, которые используются дальше по документации, будут стабильно работать для `ssh`, `rsync`, URL веб-панели и команд деплоя.
